...
1. **沿 -Z 推进的循环，条件别写反。**
   厂房 `Z0 = -30 / Z1 = -178`，写 `for (let z = Z0 + 6; z <= Z1 - 4; z -= 11)` 是**永假**。
   当时柱列 / 桁架 / 高窗 / 枕木四批构件一次都没生成，沈阳看起来「又空又暗」，
   排查了两轮才找到。正确写法是 `z >= Z1 + 4`。
2. **贴在墙上的东西要放在墙体「内表面」以内。**
   `sy_wall_l` 中心 `X0+0.15`、厚 0.3 → 内表面 `X0+0.30`。
   文字与高窗原先放在 `X0±0.02`，全在墙体内部 —— 被墙挡得干干净净，画面里什么都不剩。
   现在统一用 `X0 + 0.45 / X1 - 0.45`（离内表面 0.15，既不被挡也不 z-fighting）。
3. **共享的 `glass` 材质是 72% 透明的。**
   贴在暗墙上的高窗用它等于隐形。沈阳的高窗是这一章的主光源，
   必须换成自发光材质（`MeshBasicMaterial` + `DoubleSide`）。
4. **嵌套 group 的局部坐标要跟着父级算一遍。**
   浇包挂在 `craneHook`（世界 y=4.1）下，写成 `-6.6` 就落到 y=-2.5 ——
   整个埋在地面以下。凡是「挂在某个东西下面」的构件，都要先把父级的世界高度算清楚。
