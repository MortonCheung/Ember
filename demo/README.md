...
「炉火不灭」—— 中国工业博物馆（沈阳）Web 数字展馆。
现有版本包含展馆总览、序厅、通史馆、铸造馆与浇铸交互，支持 WebGL 降级与响应式布局。
...
| `/` / `#/` | 展馆总览（四个已开放模块的唯一入口） |
| `#/entrance` | 序厅《炉前》三维场景 |
| `#/history` | 通史馆·铁西纪年图文长卷 |
| `#/hall` | 铸造馆三维展厅 |
| `#/cast` | 亲手浇铸五步交互 |

导航采用星形结构：总览页是唯一 Hub，各模块完成后回到总览。
唯一保留的上下文深链是「铸造馆 → 亲手浇铸」，浇铸页会按来路返回铸造馆或展馆总览。
